import {
  ...
  const ProductBookingDetails = ({route }) => {
    ...
    function gst_amount() {
      return (
        (parseFloat(productData?.price) * 18.0) / 100).toFixed(2)
    }
  
    function total_amount() {
      return (
        parseFloat(parseFloat(productData?.price) + (parseFloat(productData?.price) * 18.0) / 100).toFixed(2)
      );
    }
    ...
    function bannerInfo() {
      return (
        <>
          ...
          >
            <Text style={{ color: Colors.primaryDark, fontSize: 16, fontWeight: "600" }} >Jal Abhishek product</Text>
            <Text style={{ color: Colors.grayC, fontSize: 14 }}>Jal Abhishek product Jal Abhishek product Jal Abhishek product</Text>
          </View>
        </>
      );
    }
